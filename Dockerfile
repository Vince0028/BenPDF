...
RUN echo "Checking for /app/.venv/bin/gunicorn:" && ls -l /app/.venv/bin/gunicorn || echo "ERROR: gunicorn executable NOT found in venv bin"
# Check if gunicorn module is discoverable by Python using the venv's python
RUN echo "Checking for gunicorn module discoverability:" && /app/.venv/bin/python -c "import gunicorn; print('SUCCESS: gunicorn module found using venv python')" || echo "ERROR: gunicorn module NOT found using venv python"
# --- End Debugging Steps ---
